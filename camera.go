...
const (
	...
	USB_MODE_SYNC = C.ARV_UV_USB_MODE_SYNC
	USB_MODE_ASYNC = C.ARV_UV_USB_MODE_ASYNC
)
...
func NewCamera(name string) (Camera, error) {
	...
	cs := C.CString(name)
	c.camera, err = C.arv_camera_new(cs, nil)
...
func (c *Camera) GetVendorName() (string, error) {
	name, error := C.arv_camera_get_vendor_name(c.camera, nil)
	return C.GoString(name), error
}

func (c *Camera) GetModelName() (string, error) {
	name, error := C.arv_camera_get_model_name(c.camera, nil)
	return C.GoString(name), error
}

func (c *Camera) GetDeviceId() (string, error) {
	id, error := C.arv_camera_get_device_id(c.camera, nil)
...
func (c *Camera) StartAcquisition() {
	C.arv_camera_start_acquisition(c.camera, nil)
}

func (c *Camera) StopAcquisition() {
	C.arv_camera_stop_acquisition(c.camera, nil)
}

func (c *Camera) AbortAcquisition() {
	C.arv_camera_abort_acquisition(c.camera, nil)
}

func (c *Camera) SetAcquisitionMode(mode int) {
	C.arv_camera_set_acquisition_mode(c.camera, C.ArvAcquisitionMode(mode), nil)
}

func (c *Camera) SetFrameRate(frameRate float64) {
	C.arv_camera_set_frame_rate(c.camera, C.double(frameRate), nil)
}

func (c *Camera) GetFrameRate() (float64, error) {
	fr, err := C.arv_camera_get_frame_rate(c.camera, nil)
...
func (c *Camera) SetTrigger(source string) {
	...
	C.arv_camera_set_trigger(c.camera, csource, nil)
	C.free(unsafe.Pointer(csource))
}

func (c *Camera) SetTriggerSource(source string) {
	csource := C.CString(source)
	C.arv_camera_set_trigger_source(c.camera, csource, nil)
	C.free(unsafe.Pointer(csource))
}

func (c *Camera) GetTriggerSource() (string, error) {
	csource, err := C.arv_camera_get_trigger_source(c.camera, nil)
	return C.GoString(csource), err
}

func (c *Camera) SoftwareTrigger() {
	C.arv_camera_software_trigger(c.camera, nil)
}

func (c *Camera) IsExposureTimeAvailable() (bool, error) {
	gboolean, err := C.arv_camera_is_exposure_time_available(c.camera, nil)
	return toBool(gboolean), err
}

func (c *Camera) IsExposureAutoAvailable() (bool, error) {
	gboolean, err := C.arv_camera_is_exposure_auto_available(c.camera, nil)
	return toBool(gboolean), err
}

func (c *Camera) SetExposureTime(time float64) {
	C.arv_camera_set_exposure_time(c.camera, C.double(time), nil)
}

func (c *Camera) GetExposureTime() (float64, error) {
	cdouble, err := C.arv_camera_get_exposure_time(c.camera, nil)
...
func (c *Camera) SetExposureTimeAuto(mode int) {
	C.arv_camera_set_exposure_time_auto(c.camera, C.ArvAuto(mode), nil)
}
...
func (c *Camera) SetGain(gain float64) {
	C.arv_camera_set_gain(c.camera, C.double(gain), nil)
}

func (c *Camera) GetGain() (float64, error) {
	cgain, err := C.arv_camera_get_gain(c.camera, nil)
...
func (c *Camera) GetPayloadSize() (uint, error) {
	csize, err := C.arv_camera_get_payload(c.camera, nil)
...
func (c *Camera) GVGetNumStreamChannels() (int, error) {
	cint, err := C.arv_camera_gv_get_n_stream_channels(c.camera, nil)
	return int(cint), err
}

func (c *Camera) GVSelectStreamChannels(id int) {
	C.arv_camera_gv_select_stream_channel(c.camera, C.gint(id), nil)
}

func (c *Camera) GVGetCurrentStreamChannel() (int, error) {
	cint, err := C.arv_camera_gv_get_current_stream_channel(c.camera, nil)
	return int(cint), err
}

func (c *Camera) GVGetPacketDelay() (int64, error) {
	cint64, err := C.arv_camera_gv_get_packet_delay(c.camera, nil)
	return int64(cint64), err
}

func (c *Camera) GVSetPacketDelay(delay int64) {
	C.arv_camera_gv_set_packet_delay(c.camera, C.gint64(delay), nil)
}

func (c *Camera) GVGetPacketSize() (int, error) {
	csize, err := C.arv_camera_gv_get_packet_size(c.camera, nil)
	return int(csize), err
}

func (c *Camera) GVSetPacketSize(size int) {
	C.arv_camera_gv_set_packet_size(c.camera, C.gint(size), nil)
}

func (c *Camera) GetChunkMode() (bool, error) {
	mode, err := C.arv_camera_get_chunk_mode(c.camera, nil)
...
func (c *Camera) UVSetUSBMode(mode int) {
	C.arv_camera_uv_set_usb_mode(c.camera, C.ArvUvUsbMode(mode))
}
